...
| 5 — Dashboard & integration | React dashboard, rosbridge, unified launch | Implemented, mostly demonstrated live; **exit gate run twice, not passed** |
...
**What is independently verified.** The message and service contracts match their definitions field-for-field across 13 topic subscriptions and 2 service calls, with zero mismatches; the dashboard's nine FSM state strings match the agent's `AgentState` enum exactly; the navigator's static obstacle list matches all 26 rocks in the world file; the resource map's update is a real Gaussian conjugate posterior, measured at 0.21 ms per update on the production 500 × 500 grid; and the workspace builds clean under ROS 2 Jazzy (all six packages, zero errors). The Python suite is **947 passing in a single pytest process** spanning all five Python packages, in either collection order, with one declared skip (`selene_hal`'s Gazebo backend, which needs a real `rclpy`). The dashboard now has a JavaScript test runner for the first time — **39 Jest tests** over the real state reducer and the real canvas mark planner. All measured on Python 3.11.6 / pytest 9.1.1 on 2026-07-31. **One documented lane is currently red** and is tracked as deviation D-36: `selene_orchestrator/test` alone fails on an unguarded `selene_agent` import.
...
**What is not.** Most of Phase 5 has now been observed on a running system, and `docs/phase5_deviation_register.md` states per deviation which claims rest on a live run and which on a unit test — it remains the authority, and it draws the "implemented" versus "demonstrated" line that this paragraph summarises. What is still missing: **RViz2 has never been started**, so the side-by-side comparison of the overlay against the dashboard heatmap that `docs/PRD.md:1504` asks for has never been performed; PRD row 7 (dashboard frame timing) is not coverable headlessly; and two of the gate's seven PRD rows were **not measured at all** in either run, because the gate cannot observe an FSM state that lasts 0.25 s through a 0.5 s sampler. **The cause of a reproducible Gazebo/ODE `collide()` abort — three crashes on 2026-07-30/31 — is unknown.** It has not recurred in 24,327 abort-free robot-seconds since, and four things changed at once, so that is not a fix. The launch now dies with the simulator instead of running on over dead physics, which makes the next one loud. **CI has been failing since 2026-04-08** — the known causes are fixed, but a green pipeline has not yet been observed. Do not treat CI as a signal until it goes green on its own.
...
2. **The planner's obstacle world and the simulator's agree** — all 26 obstacle positions are identical between `selene_sim/`'s world file and `selene_agent/config/nav_params.yaml`, and `selene_sim/test/test_world_extent_agrees.py` fails the build if the terrain extent stops agreeing across the four files that declare it. **This does not extend to every mission coordinate**: the recharge position is declared three times with three different values (`agent_node.py`'s parameter default, `nav_params.yaml`, and `energy_manager`'s constructor), and only one of them is read. Tracked as deviation D-32.
...
6. **The combined test suite is order-independent** — 947 passing in one pytest process spanning all five Python packages, identical totals in both collection orders. This broke once (D-14: `conftest.py` installed process-global ROS stubs that were incomplete relative to what `selene_hal` imports, and a combined run aborted at collection having run zero tests) and the fix is structural rather than a patched stub: the stubs are inserted into `sys.modules` only while pytest is collecting or running something under `selene_orchestrator/test`, and removed on the way out. Enforced by `selene_hal/test/test_ros_stub_isolation.py` and the `cross-package-tests` CI job. **The converse does not currently hold**: `selene_orchestrator/test` run *alone* fails, because one test imports `selene_agent` without a guard. That is deviation D-36, open.
...
- [ ] **Phase 5 — Dashboard & integration.** Code complete and mostly demonstrated on a live system; **exit gate run twice on 2026-07-31 and not passed** (exit 1, then exit 2). See `docs/phase5_deviation_register.md`.
- [ ] **Phase 6 — Polish & hardening.** Integration demos, NFR performance validation, 30-minute stability run. Not started as a phase — though a 30-minute ten-robot run did complete at real-time factor 1.000 on 2026-07-31, and the hardening work on branch `phase5-hardening` closed eleven deviations.
...
- **The Phase 5 exit gate has been run twice and does not pass.** 8 passed / 1 failed / 2 skipped (exit 1), then 9 / 0 / 2 (exit 2). Two checks SKIP on a *correct* system because the gate cannot sample an FSM state shorter than 0.5 s (D-34); one check flips between FAIL and PASS on identical invocations, separated by 33 cm of displacement (D-35). Neither was patched. `docs/phase5_validation_report.md` still records one run of the *superseded* eight-check gate at commit `251e84d` and its footer asserts a waiver that is now false; it must be regenerated by a green run, never hand-edited.
- **`navigation.max_traversable_slope_deg` has had no reader since Phase 2.** It is declared in two config files and read only by a test. The planner does not consult it, so an operator-injected target on a 35° crater wall would be planned and driven with nothing refusing it. See D-28.
...
# Counts measured 2026-07-31 on Python 3.11.6 / pytest 9.1.1.
...
# Or in lanes. The extra skip is selene_sim's cross-check against the HAL,
# which stands down when selene_hal is not on this lane's path.
...
# NOTE: selene_orchestrator/test run WITHOUT selene_agent on the path is
# currently RED — test_terrain_guard.py imports selene_agent unguarded.
# Deviation D-36, open.
...
cd selene_dashboard && npm ci && npx eslint src \
  && CI=true npx react-scripts test --watchAll=false                       # 39 passed, 2 suites
npm run build
```

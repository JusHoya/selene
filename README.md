...
![status](https://img.shields.io/badge/status-Sprint%200%20%C2%B7%20Phase%205%20gate%20green%20%C2%B7%20Phase%206%20not%20started-6E56CF?style=flat-square)
...
> [!IMPORTANT]
> **Sprint 0 prototype. Phases 1–5 are implemented, and the Phase 5 exit gate PASSES.** `scripts/validate_phase5.sh` has been run **ten** times; runs 9 and 10 (2026-08-01, ROS 2 Jazzy + Gazebo Harmonic 8.11.0) are both **11 passed / 0 failed / 0 skipped, exit 0** — the first greens in the gate's history. The row that had blocked it for eight runs was closed by **a decision rather than a fix**: an operator injection tagged `emergency` may abort an auction already in flight (D-44). Along the way the fleet ran end to end and delivered 94.85 kg to the depot, the dashboard was confirmed rendering in a browser, the RViz2 side-by-side the PRD asks for was performed, and the fleet's real slope capability was measured over 54 Gazebo trials.
>
> **Read the caveats with the green, because two of the eleven rows measure proxies and the gate says so itself:** check 10 runs on a map **the gate seeds**, so it does not show that robots autonomously survey; **neither green run measured a preemption** (check 6's corroboration clause reported NOT APPLICABLE both times); and PRD row 7 (dashboard frame timing) is **NOT COVERED** — no browser is started anywhere in the gate. **One deviation is open with an unknown cause**: a reproducible Gazebo/ODE abort (D-37), where the 45-minute campaign that cleared both hazard models ran a configuration that is *not the one that crashed*.
>
> **Phase 6 has not started, and nine of its ten exit-gate rows are unstarted or partial** — there is no harness for any of the PRD's four integration demos, no performance report, no coverage tooling, no `launch_testing` test and no rosbag support. Everything in [Architecture](#architecture) exists as code; read [Status](#status) and `docs/phase5_deviation_register.md` before relying on any single capability.
...
| 5 — Dashboard & integration | React dashboard, rosbridge, unified launch | Implemented and demonstrated live; **exit gate run ten times — runs 9 and 10 both 11/0/0, exit 0** |
| 6 — Polish & hardening | Stability, performance, integration demos | **Not started.** Nine of its ten exit-gate rows are unstarted or partial; the tenth (CI green) is satisfied at HEAD |

**What is independently verified.** The message and service contracts match their definitions field-for-field across 13 topic subscriptions and 2 service calls, with zero mismatches; the dashboard's nine FSM state strings match the agent's `AgentState` enum exactly; the navigator's static obstacle list matches all 26 rocks in the world file; the resource map's update is a real Gaussian conjugate posterior, measured at 0.21 ms per update on the production 500 × 500 grid; and the workspace builds clean under ROS 2 Jazzy (all six packages, zero errors). The Python suite is **1394 passing in a single pytest process** spanning all five Python packages, in either collection order, with one declared skip (`selene_hal`'s Gazebo backend, which needs a real `rclpy`). The gate lane — `selene_orchestrator` + `selene_isru` alone — is **815 passing, 3 skipped**, and CI now runs it whole. The dashboard's JavaScript suite is **122 Jest tests** over 9 suites, exercising the real state reducer, the real canvas mark planner and the resource view's canvas lifecycle. All re-measured on Python 3.11.6 on 2026-08-02 against commit `4efb47d`. (These read 1150 / 623 / 101 until 2026-08-02; the +166 on both pytest lanes is D-44's emergency-preemption work.) **No documented lane is red**: D-36 is fixed and now has a CI job (`gate-lane-tests`) that was mutation-tested before it landed. **SELENE CI is green on `main` — 9/9 jobs at HEAD** (`4efb47d`, run `30731928511`, push to `main`), including the ROS 2 Humble `colcon build`/`colcon test` lane; the separate real-Gazebo **Simulation gates** workflow is green at the same commit twice, once on a schedule.

**Code coverage, measured for the first time on 2026-08-02:** 66% of statements repo-wide, **93%** with the nine rclpy node shells excluded, and every module the PRD names as core logic at or above 87% (`fsm` 99, `task_auction` 98, `energy_manager` 97, `htn_planner` 96, `resource_map` 87). NFR-5.1's 80% bar is probably already cleared on core logic — but **there is no coverage tooling in the repository or in CI**, so that figure is not reproducible from a checkout, and "core logic modules" has never been pinned to a file list. Both are Phase 6 work.
...
**What is not.** Most of Phase 5 has now been observed on a running system, and `docs/phase5_deviation_register.md` states per deviation which claims rest on a live run and which on a unit test — it remains the authority, and it draws the "implemented" versus "demonstrated" line that this paragraph summarises. What is still missing: **PRD row 7 (dashboard frame timing) is not coverable headlessly** — no browser is started anywhere in the exit gate. **Check 10 runs on a map the gate seeds itself** (49 synthetic readings shaped like the real deposits), so it proves the fusion → sparse-encode → marker path and **not** that robots autonomously survey the deposits. **Neither green gate run measured an auction preemption** — check 6's corroboration clause reported NOT APPLICABLE on both — so the emergency path is demonstrated separately, by `scripts/demo_emergency_preemption.py` against its own control, and **no gate row exercises the non-emergency injection path at all**. The two PRD rows that no run had ever measured — the gate could not observe an FSM state lasting 0.25 s through a 0.5 s sampler — **were measured on 2026-08-01 and both passed**. **The cause of a reproducible Gazebo/ODE `collide()` abort — three crashes on 2026-07-30/31 — is unknown**; it has not recurred in 24,327 abort-free robot-seconds, but the 45-minute campaign that cleared both hazard models ran **a configuration that is not the one that crashed** (no agents, no orchestrator, no skills), so that is not a fix. (The RViz2 side-by-side `docs/PRD.md:1504` asks for **has now been performed**, against a map the fleet surveyed rather than a seeded one; and the 0.0 % battery report was root-caused to a second `battery_node` on a shared ROS domain — D-42, closed.) Two further limits are worth knowing before a demo: **no vehicle has driven a switchback** (the routes that make the crater reachable are executed plans, not journeys), and **side-slope rollover — usually the binding limit on a real rover — is unmeasured and unbounded**. Browser observations made on 2026-08-01 were against a **rosbridge test double, not the real stack**.
...
Excerpted verbatim from `selene_hal/config/scout.yaml`. **Corrected 2026-08-02:
this block read `capacity: 500` and `locomotion_draw: 20` and omitted
`idle_draw` — wrong by 10× and 7.5× against the real descriptor, which put
scout endurance at 25 h instead of 50 / (10 + 150 × 0.5) = 0.59 h. The one
number that decides whether the PRD's energy-management demo is plausible was
out by 42×, in the only RCDL example the project publishes.**
...
6. **The combined test suite is order-independent** — 1394 passing in one pytest process spanning all five Python packages, identical totals in both collection orders. This broke once (D-14: `conftest.py` installed process-global ROS stubs that were incomplete relative to what `selene_hal` imports, and a combined run aborted at collection having run zero tests) and the fix is structural rather than a patched stub: the stubs are inserted into `sys.modules` only while pytest is collecting or running something under `selene_orchestrator/test`, and removed on the way out. Enforced by `selene_hal/test/test_ros_stub_isolation.py` and the `cross-package-tests` CI job. **The converse now holds too**: `selene_orchestrator/test` run on the two-package path is green (815 passed, 3 skipped), every skip is `importorskip`-guarded, the cross-package lane still runs all of those assertions with zero skips, and a `gate-lane-tests` CI job runs the lane whole. That was deviation D-36, closed.
...
- [x] **Phase 5 — Dashboard & integration.** Code complete, demonstrated on a live system, and **the exit gate PASSES — runs 9 and 10 are both 11 passed / 0 failed / 0 skipped, exit 0** (2026-08-01). Two of the eleven rows measure proxies and the gate's own report names them. See `docs/phase5_deviation_register.md`.
- [ ] **Phase 6 — Polish & hardening.** Integration demos, NFR performance validation, 30-minute stability run. **Not started**, and larger than it looks: there is no harness for any of the PRD's four integration demos, no performance report, no coverage tooling, no `launch_testing` test and no rosbag support, so **nine of the ten Phase 6 exit-gate rows are unstarted or partial**. The tenth — CI green on lint + build + test — is satisfied at HEAD. Of the twenty-six deviations opened by running the system (D-19..D-44), **twenty-five are closed**; the one that remains, **D-37**, has no known cause. A 30-minute ten-robot run did complete at real-time factor 1.000 on 2026-07-31, but it predates the D-43 path-follower fix and delivered 94.85 kg against Demo 1's 100 kg.
...
- **The Phase 5 exit gate has been run ten times and it PASSES.** Runs 1–8 were 8/1/2 (exit 1), 9/0/2 (exit 2), 10/1/0 (exit 1) and — runs 4–8 — a sequence ending at 9/1/1 (exit 1). **Runs 9 and 10, 2026-08-01 at `d7f0f3e`, are both 11 passed / 0 failed / 0 skipped, exit 0**: the first greens in this gate's history, on ROS 2 Jazzy / Gazebo Harmonic 8.11.0 with a 2/1/1 fleet. The two checks that used to SKIP on a *correct* system now produce verdicts and both pass (D-34), and the check that used to flip between FAIL and PASS now aims at a bearing derived from the robot's own heading with a window derived from its descriptor (D-35 — the 12 s literal was deleted, not raised). What finally closed the last failing row was **a decision, not a fix**: an operator injection tagged `emergency` may abort an auction already in flight, while a non-emergency priority-10 injection keeps the old wait-your-turn behaviour (D-44). `docs/phase5_validation_report.md` **is run 9** and is current. (It recorded the *superseded* eight-check gate at `251e84d` until 2026-08-01; this bullet said so until 2026-08-02, a day after it stopped being true.) It must be regenerated by a green run, never hand-edited. **Read the register's D-44 gate block before quoting the green** — two of the eleven rows measure proxies and say so.
- **A scout reported 0.0 % battery six seconds after startup, three times — and the cause was found: a second `battery_node` for the same robot on a shared ROS domain.** `ros2 launch` does not take its children with it, so an incomplete teardown leaves a complete second SELENE stack alive; the orphaned node latches its last `cmd_vel` and keeps integrating locomotion draw with no simulator behind it, reaching the clamp in 28.5 minutes and publishing exactly 0.0 thereafter. Root-caused, reproduced on demand, fixed, and the fix demonstrated against the reproduction. **The general hazard remains and this system has little defence against it**: nothing sets `ROS_DOMAIN_ID` outside `docker-compose.yaml`, `GZ_PARTITION` isolates Gazebo transport only, and no message carries producer identity — so a contaminated domain looks exactly like one node behaving erratically. See D-42.
...
cd selene_dashboard && npm ci && npx eslint --ext .js,.jsx src \
  && CI=true npx react-scripts test --watchAll=false                       # 122 passed, 9 suites
npm run build
```

...
The candidate grid is laid out over the *bounding box* of the PSR circle, then filtered. For the default configuration --- PSR radius $R = 60$ m, candidate resolution $\Delta = 5$ m --- the bounding box yields $(2R/\Delta + 1)^2 = 25^2 = 625$ grid points. The PSR boundary test $\|\mathbf{x}_k - \mathbf{c}\|_2 \leq R$ retains 441 of these (70.6%; the continuum limit would be $\pi/4 \approx 78.5\%$, but the discrete grid loses more at the corners). With the default PSR centre $(-100, -150)$ and the $500 \times 500$ m map spanning $[-250, 250]^2$, the map-bounds test removes no further candidates. So the initial candidate set is 441 cells, enumerated by `_generate_candidates` in `selene_orchestrator/selene_orchestrator/adaptive_survey.py:107`. The minimum-spacing filter progressively reduces this count as the survey advances. When the candidate set is exhausted ($\lvert\mathcal{C}\rvert = 0$), the planner returns `None`, signaling to the orchestrator that the survey zone has been fully covered.
...
**Integration status.** The closed loop described above is the intended design, but it is *not yet wired into the running system*. The planner is implemented and unit-tested in isolation (`selene_orchestrator/test/test_adaptive_survey.py`, 8 test cases exercising scoring, spacing, exhaustion, and weight sensitivity), and `AdaptiveSurveyPlanner` is instantiated by `OrchestratorNode.__init__` in `selene_orchestrator/selene_orchestrator/orchestrator_node.py` --- but the resulting `self._adaptive_survey` attribute appears nowhere else in that file, so no method on the instance is ever called, and `select_next_waypoint` has no call site outside its own tests. In the current build, all survey waypoints come from the static hexagonal grid computed once at mission decomposition by `_generate_survey_waypoints` in `selene_orchestrator/selene_orchestrator/htn_planner.py`, capped at `SURVEY_WAYPOINT_COUNT = 10` (`htn_planner.py:28`). Closing the loop --- having the HTN planner call `select_next_waypoint` after each survey completion so that the next waypoint reflects the updated posterior --- is the remaining integration work and is not yet validated in simulation. The *sequential Bayesian experimental design* framing therefore describes the architecture, not measured behaviour.


# 8. Comparison with Pure Information-Gain and Static Grid Approaches

We compare three survey strategies analytically and by inspection of the implemented scoring function, within a 120 m diameter PSR (radius 60 m, matching the default in `adaptive_survey.py:30` and `selene_sim/config/world_params.yaml`) containing two synthetic ice deposits (Gaussian blobs with peak concentrations of 6.0 and 8.0 wt%, spatial standard deviations of 10 m and 8 m respectively). **No head-to-head simulation benchmark of the three strategies exists in the repository**; the comparison below is a design argument, and the behavioural descriptions are expectations derived from the scoring function rather than measured results.

## 8.1 Static Hexagonal Grid

The baseline strategy generates a fixed hexagonal tessellation with 20 m spacing --- this is what SELENE's HTN planner actually uses today for survey waypoints (`SURVEY_SPACING = 20.0`, `htn_planner.py:31`). Waypoints are sorted by distance from the PSR center and visited in order. At the default 60 m PSR radius with a 5 m boundary margin, the full tessellation contains 27 waypoints, but the HTN planner caps the survey at the closest `SURVEY_WAYPOINT_COUNT = 10` (`htn_planner.py:28`). The deployed baseline is therefore a 10-waypoint partial hex sample, not full-coverage tessellation.
...
**Expected behavior.** The planner should initially explore broadly (variance-dominant phase). Upon detecting the first deposit, the neighbor signal term activates, and subsequent waypoints should cluster around the detection while still periodically visiting high-variance regions elsewhere. The distance term biases selection toward nearby candidates, favouring shorter traverses. The unit test `test_convergence_near_deposits` (`selene_orchestrator/test/test_adaptive_survey.py:105`) confirms the clustering behaviour on a synthetic map; the broader multi-phase narrative is a design expectation and has not been observed in a full simulation run.

**Quantitative comparison --- not yet measured.** We are not able to report waypoint counts, energy savings, or a percentage reduction relative to static coverage, because no comparative benchmark has been run: there is no benchmark harness in the repository, and the adaptive planner is not yet invoked by the orchestrator (Section 7). The mechanism by which a saving would arise is clear --- the static baseline visits a fixed 10-waypoint hex sample regardless of what it finds, while the adaptive planner redirects sampling toward the deposit it has already detected --- but quantifying it requires the integration and simulation work described in Section 7. Establishing these numbers is the primary open experimental task for this contribution.
...
The default configuration $w_v = 1.0$, $w_s = 0.5$, $w_d = 0.3$ (`adaptive_survey.py:31`--`33`) was selected to produce a moderate exploration bias with secondary exploitation and mild distance preference. The intent is a planner that discovers multiple deposits, characterizes their boundaries with a small number of follow-up waypoints, and keeps traverses short. The sensitivity statements above follow from the structure of the normalized scoring function; the specific tuning has not been swept against a measured objective, and no traverse-efficiency figure has been established.
...
The planner is designed to close a loop with the Bayesian resource map: observations update the map, the map updates the scoring landscape, and the scoring landscape directs future observations. We state plainly that this loop is not yet closed in the SELENE build. The planner is implemented and unit-tested against synthetic maps, and it is instantiated by the orchestrator, but no orchestrator code path calls it; survey waypoints still come from a fixed 10-point hexagonal sample generated once at mission decomposition. Consequently the expected reduction in waypoints relative to static coverage is an argued benefit, not a measured one. Integrating the planner into the HTN survey phase and then measuring waypoints-to-detection against the static baseline in Gazebo is the necessary next step.

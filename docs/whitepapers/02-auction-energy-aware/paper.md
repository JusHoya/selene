...
paper-number: "WP-02"
abstract: |
  Multi-robot task allocation in lunar environments must reconcile three competing concerns absent from terrestrial domains: severe energy constraints with no ambient recharging, communication latency that precludes synchronous negotiation, and heterogeneous robot capabilities spanning the full ISRU value chain. We present a market-based auction protocol with energy-aware bid scoring, implemented within the SELENE fleet coordination system. Each auction cycle broadcasts a task announcement, collects bids within a delay-tolerant timeout window, and selects the winner via a weighted scoring function that integrates spatial proximity, round-trip energy affordability, and binary capability matching. The energy model computes a complete go-execute-return budget with a 10% safety margin, ensuring that no robot accepts a task it cannot complete while retaining sufficient charge to reach a recharging station. We prove that the protocol satisfies individual rationality, maintains liveness under communication loss, and achieves a 2-approximation bound relative to the energy-optimal assignment. The mechanism is integrated end to end in the orchestrator and agent nodes and is covered by the Python unit-test suite; it has **not** yet been validated in Gazebo Harmonic, and this paper reports no measured performance figures --- the analytical results stand on the proofs, not on simulation data.
...
The mechanism is compact: 62 lines for the `TaskAuction` round state machine (`selene_orchestrator/selene_orchestrator/task_auction.py`) and 145 lines for the agent-side `EnergyManager` (`selene_agent/selene_agent/energy_manager.py`), with announcement, bid construction, and winner selection inline in the orchestrator and agent nodes. It is integrated end to end and unit-tested. It has not yet been exercised in Gazebo Harmonic with the four-robot fleet; that run is the outstanding validation step.
...
Because `_auction_tick` returns early whenever an auction is already active, throughput is set by the timeout rather than by the tick rate: the 5.0 s bid window plus up to one 0.5 s tick of detection latency gives one allocation every 5.0--5.5 s, i.e. roughly 0.18--0.20 tasks/s. The 2 Hz tick rate governs only how promptly a completed or newly-eligible auction is noticed. This allocation rate is ample for ISRU operations where individual tasks span minutes to hours, but it does bound fleet start-up: seeding 10 survey tasks takes on the order of a minute of wall-clock auctioning.
...
where $d_{ij} = \|\mathbf{p}_i - \mathbf{q}_j\|_2$ is the Euclidean distance from robot $r_i$ to task $\tau_j$, and $\sigma_d = 100$ m is a characteristic length scale. This function maps $d = 0$ to $f_d = 1.0$ and decays monotonically, reaching $f_d = 0.5$ at $d = \sigma_d$. The inverse-linear form was selected over the Gaussian $\exp(-d^2 / 2\sigma^2)$ because the Gaussian's rapid decay beyond $\sigma$ drives the proximity term to numerically negligible values at ranges the fleet routinely traverses, suppressing bids that ought to be competitive. The scale is easy to check against the world: the depot sits at $(-30, -100)$ and the PSR is centred at $(-100, -150)$ with a 60 m radius (`selene_sim/config/world_params.yaml`), so depot-to-survey-waypoint distances run from roughly 30 m to 150 m. Relative to the near edge of that range, a Gaussian with $\sigma = 100$ m weights the far edge at $0.34$, whereas the deployed inverse-linear form weights it at $0.52$ --- a materially flatter penalty that keeps distant-but-affordable robots in contention. We record this as the stated design rationale; no A/B measurement of fleet utilization under the two forms exists in the repository.
...
The energy model assumes constant-speed traversal at $v = 0.3$ m/s, which is representative of lunar rover locomotion over regolith terrain. Power draws are robot-specific: for example, the scout class draws 10 W idle and 150 W/(m/s) locomotion against a 50 Wh battery, while the excavator draws 15 W idle and 250 W/(m/s) locomotion against an 80 Wh battery (`selene_hal/config/scout.yaml`, `selene_hal/config/excavator.yaml`).
...
SELENE sets $T_{\text{auction}} = 5.0$ seconds (`auction_timeout_sec` in `selene_orchestrator/config/orchestrator_params.yaml`), providing roughly a 20$\times$ margin over the estimated lower bound. This generous margin serves a specific design purpose: it accommodates transient DDS endpoint re-discovery delays that occur when robots rejoin the network after a communication blackout, a process that is orders of magnitude slower than steady-state message exchange and is not bounded by the RTT figures above. We have not measured re-discovery time for this system --- no timing artifact exists in the repository --- so 5.0 s should be read as a conservative engineering margin chosen ahead of measurement, not as a value calibrated against observed data. Measuring post-blackout re-discovery and tightening or widening the window accordingly is open work.
...
We expect this bound to be loose for a fleet of four, since the cascading displacement that drives the worst case requires a longer chain of contested tasks than four robots can produce. We are not able to quantify the gap: no comparison of the greedy allocation against an offline optimum has been run, and the repository contains no benchmark harness that would produce one. Measuring the realised approximation ratio is open work.
...
We have presented the market-based task allocation mechanism implemented in the SELENE lunar ISRU fleet coordination system. The protocol combines a sequential single-item auction with an energy-aware bid scoring function that integrates spatial proximity, round-trip energy affordability with a 10% safety margin, and capability matching. The design is deliberately simple --- 62 lines for the `TaskAuction` class and 145 for the `EnergyManager` module, plus bidding logic inline in the agent node --- reflecting a conviction that reliability in safety-critical space systems is better served by analyzable simplicity than by algorithmic sophistication. The claims in this paper are analytical: the three propositions are proved, and the implementation is unit-tested, but no simulation measurements are reported because none have been taken.

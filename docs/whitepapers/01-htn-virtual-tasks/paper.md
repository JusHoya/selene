...
The complete implementation comprises 511 lines of pure Python (349 lines for the HTN planner in `htn_planner.py`, 162 for the task queue in `task_queue.py`) with no ROS dependencies, enabling unit testing, formal analysis, and deployment flexibility. Current work focuses on evaluation in the SELENE Gazebo simulation environment with heterogeneous rover fleets --- not yet completed --- and on two integrations that are designed but not wired: the adaptive survey planner (WP-04), which the planner does not currently call, so survey waypoints remain a fixed 10-point hexagonal sample; and the material conservation ledger (WP-06), which `check_and_advance` does not currently query, instead inferring deposited mass as completed-haul-count $\times$ nominal hopper capacity. Extension of the virtual task mechanism to support branching decompositions, where alternative extraction strategies are selected based on resource map characteristics, remains future work.

...
_Generated Sat Aug  1 03:09:18 PM CDT 2026_

| | |
|---|---|
| Source commit | `0cf8080 (working tree dirty)` |
...
|---|---|---|---|
| 1 | Single launch command starts the full system | PASS | 23 expected nodes present, sim_time 15764ms -> 18914ms, 4 Gazebo models, 5 topics with a live publisher (ready after 15s) |
...
| 4 | Robot state content, freshness, rate and fleet membership | PASS | 4 robots, excavator_01 2.00 Hz, hauler_01 2.00 Hz, scout_01 2.06 Hz, scout_02 2.05 Hz; scout_01 NAVIGATING moved 36.17 m over 149 samples; scout_02 NAVIGATING moved 43.53 m over 180 samples; rosapi topics_for_type agrees with the fleet |
...
| 6 | Injected task is announced and assigned, correlated by task_id | FAIL | task manual_0000 was announced 2.91s after injection but was never assigned within the following 15s. 1 assignment(s) of OTHER tasks were seen in that window — the auction ran and this task did not win it (no robot became IDLE in 60s, so scout_01 was freed with an operator cancel_task first — this row was measured on a robot the gate perturbed. the accepted cancel_task response is itself the receipt: the agent returns accepted only after firing OPERATOR_CANCEL, which is an unconditional transition to IDLE; corroborated by an IDLE state sample after the cancel) |
...
**Summary:** 9 passed, 1 failed, 1 skipped.

## What this run does and does not mean

**This run was not green** — 9 passed, 1 failed,
...
**Probe stderr:** /tmp/selene_phase5_probe.22740.log
**Probe measurements (JSON):** /tmp/selene_phase5_probe.22740.json
